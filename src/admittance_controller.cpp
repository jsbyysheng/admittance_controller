...
admittance_control::admittance_control(
        ...
        std::vector<double> joint_limits, std::vector<double> maximum_velocity,
        ...
        joint_limit(joint_limits.data()), max_vel(maximum_velocity.data()), max_acc(maximum_acceleration.data()) {

    // ---- LOAD PARAMETERS ---- //
    ros::NodeHandle paramNodeHandle(ros::this_node::getName());

    if (!nh.param<bool>("/admittance_controller_Node/use_feedback_velocity", use_feedback_velocity, false)) {
        ROS_ERROR("Couldn't retrieve the Feedback Velocity value.");
    }
    if (!nh.param<bool>("/admittance_controller_Node/inertia_reduction", inertia_reduction, false)) {
        ROS_ERROR("Couldn't retrieve the Inertia Reduction value.");
    }
    if (!nh.param<bool>("/admittance_controller_Node/use_ur_real_robot", use_ur_real_robot, false)) {
        ROS_ERROR("Couldn't retrieve the Use Real Robot value.");
    }
    if (!nh.param<bool>("/admittance_controller_Node/auto_start_admittance", admittance_control_request,
                        true)) { ROS_ERROR("Couldn't retrieve the Auto Start Admittance value."); }
    ...
    // ---- ROS SERVICE SERVERS ---- //
    admittance_controller_activation_service = nh.advertiseService(
            "/admittance_controller/admittance_controller_activation_service",
            &admittance_control::Admittance_Controller_Activation_Service_Callback, this);
    change_admittance_parameters_service = nh.advertiseService(
            "/admittance_controller/change_admittance_parameters_service",
            &admittance_control::Change_Admittance_Parameters_Service_Callback, this);
    ur_freedrive_mode_service = nh.advertiseService("/admittance_controller/ur_freedrive_mode_service",
                                                    &admittance_control::FreedriveMode_Service_Callback, this);
    ur_restart_urcap_service = nh.advertiseService("/admittance_controller/ur_restart_urcap_service",
                                                   &admittance_control::Restart_URCap_Service_Callback, this);
    ...
    ur_zero_ft_sensor_client = nh.serviceClient<std_srvs::Trigger>("/ur_hardware_interface/zero_ftsensor");
    ur_resend_robot_program = nh.serviceClient<std_srvs::Trigger>("/ur_hardware_interface/resend_robot_program");
    ur_play_urcap = nh.serviceClient<std_srvs::Trigger>("/ur_hardware_interface/dashboard/play");

    // ---- ROS ACTIONS ---- //
    trajectory_client = new actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction>(
            topic_action_trajectory_publisher, true);
    ...
    if (simple_debug) {
        ...
        ROS_INFO_ONCE("Maximum Velocity:     %.6f %.6f %.6f %.6f %.6f %.6f", max_vel[0], max_vel[1], max_vel[2],
                      max_vel[3], max_vel[4], max_vel[5]);
        ROS_INFO_ONCE("Maximum Acceleration: %.6f %.6f %.6f %.6f %.6f %.6f \n", max_acc[0], max_acc[1], max_acc[2],
                      max_acc[3], max_acc[4], max_acc[5]);
        ROS_INFO_ONCE("Force Dead Zone:   %.6f", force_dead_zone);
        ROS_INFO_ONCE("Troque Dead Zone:  %.6f", torque_dead_zone);
        ROS_INFO_ONCE("Admittance Weight: %.6f", admittance_weight);
...
void admittance_control::force_sensor_Callback(const geometry_msgs::WrenchStamped::ConstPtr &msg) {
    ...
    ROS_INFO_NAMED(
            "FTSensor",
            "Sensor Force/Torque  ->  Fx: %.6f  Fy: %.6f  Fz: %.6f  |  Tx: %.6f  Ty: %.6f  Tz: %.6f",
            external_wrench[0],
            external_wrench[1],
            external_wrench[2],
            external_wrench[3],
    ...
    if (complete_debug) {
        for (int i = 0; i < 6; i++) {
            ft_sensor_debug << external_wrench[i] << ",";
        }
        ft_sensor_debug << "\n";
    }
    ...
    for (int i = 0; i < 3; i++) {
        if (fabs(external_wrench[i]) < fabs(force_dead_zone)) {
            external_wrench[i] = 0.0;
        }
    }
    for (int i = 3; i < 6; i++) {
        if (fabs(external_wrench[i]) < fabs(torque_dead_zone)) {
            external_wrench[i] = 0.0;
        }
    }
    ROS_INFO_NAMED(
            "FTSensor",
            "Sensor Force/Torque Clamped  ->  Fx: %.6f  Fy: %.6f  Fz: %.6f  |  Tx: %.6f  Ty: %.6f  Tz: %.6f",
            external_wrench[0],
            external_wrench[1],
            external_wrench[2],
            external_wrench[3],
    ...
    ROS_INFO_NAMED(
            "FTSensor",
            "Sensor Force/Torque LPF  ->  Fx: %.6f  Fy: %.6f  Fz: %.6f  |  Tx: %.6f  Ty: %.6f  Tz: %.6f",
            external_wrench[0],
            external_wrench[1],
            external_wrench[2],
            external_wrench[3],
...
void admittance_control::joint_states_Callback(const sensor_msgs::JointState::ConstPtr &msg) {
    joint_state = *msg;
    // UR Real Robot has Inverted Joints
    ...
    // Convert Joint Position in Cartesian (geomerty_msgs::Pose)
    publish_cartesian_position();

    // ---- DEBUG ---- //
    if (complete_debug) {
        ROS_INFO_THROTTLE_NAMED(
                2,
                "Joint Position", "joint position: %.6f %.6f %.6f %.6f %.6f %.6f",
...
void admittance_control::trajectory_execution_Callback(const ur_admittance_controller::joint_trajectory::ConstPtr &msg) {
    ROS_INFO("Trajectory Execution Callback");
...
bool admittance_control::Change_Admittance_Parameters_Service_Callback(ur_admittance_controller::parameter_srv::Request &req,
                                                                  ur_admittance_controller::parameter_srv::Response &res) {
...
Eigen::MatrixXd admittance_control::compute_arm_jacobian(std::vector<double> joint_position, std::vector<double> joint_velocity) {
    ...
    ROS_INFO_STREAM_NAMED(
            ...
            "Manipulator Jacobian: " << std::endl << std::endl << jacobian << std::endl
    );

    ROS_INFO_STREAM_NAMED(
            "Manipulator Inverse Jacobian",
            "Manipulator Inverse Jacobian: " << std::endl << std::endl << jacobian.inverse() << std::endl
    );
    // ---- DEBUG ---- //
    if (complete_debug)
        ROS_INFO_STREAM_THROTTLE_NAMED(2, "Manipulator Jacobian",
                                       "Manipulator Jacobian: " << std::endl << std::endl << jacobian << std::endl);
    if (complete_debug)
        ROS_INFO_STREAM_THROTTLE_NAMED(2, "Manipulator Inverse Jacobian",
                                       "Manipulator Inverse Jacobian: " << std::endl << std::endl << jacobian.inverse()
                                                                        << std::endl);
    return jacobian;
}
...
Matrix6d admittance_control::get_ee_rotation_matrix(std::vector<double> joint_position, std::vector<double> joint_velocity) {
    ...
    if (complete_debug) {
        ...
        ROS_INFO_THROTTLE_NAMED(2, "Translation Vector", "Translation Vector   ->   X: %.3f  Y: %.3f  Z: %.3f",
                                end_effector_state.translation().x(), end_effector_state.translation().y(),
                                end_effector_state.translation().z());
        ROS_INFO_THROTTLE_NAMED(2, "Euler Angles", "Euler Angles         ->   R: %.3f  P: %.3f  Y: %.3f",
                                euler_angles[0], euler_angles[1], euler_angles[2]);
        ROS_INFO_THROTTLE_NAMED(2, "Rotation Quaternion",
                                "Rotation Quaternion  ->   X: %.3f  Y: %.3f  Z: %.3f  W: %.3f", rotation_quaternion.x(),
                                rotation_quaternion.y(), rotation_quaternion.z(), rotation_quaternion.w());

        ROS_INFO_STREAM_THROTTLE_NAMED(2, "Rotation Matrix from Model",
                                       "Rotation Matrix from Model:" << std::endl << std::endl
                                                                     << end_effector_state.rotation() << std::endl);
        ROS_INFO_STREAM_THROTTLE_NAMED(2, "Rotation Matrix 6x6",
                                       "Rotation Matrix 6x6:" << std::endl << std::endl << rotation_matrix
                                                              << std::endl);
    }
...
void admittance_control::compute_admittance() {
    ros::spinOnce();
    // Compute Manipulator Jacobian
    ROS_INFO_NAMED(
            "joint_position",
            "joint_position:  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            joint_position[0],
            joint_position[1],
            joint_position[2],
            joint_position[3],
    ...
    ROS_INFO_NAMED(
            "joint_velocity",
            "joint_velocity:  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            joint_velocity[0],
            joint_velocity[1],
            joint_velocity[2],
            joint_velocity[3],
    ...
    /**
     * mode 1: use feedback velocity
     * mode 2: use last velocity
     * **/
    if (use_feedback_velocity) {
        ...
        ROS_INFO_STREAM_NAMED(
                "Start Velocity",
                "Start Velocity: " << std::endl << std::endl << x_dot << std::endl
        );
        // ---- DEBUG ---- //
//        if (complete_debug) {
...
//        }
    ...
    // Todo: why ftsensor_start_offset is even not inited?
    for (unsigned i = 0; i < external_wrench.size(); i++) {
        external_wrench[i] += -ftsensor_start_offset[i];
    }

    // Compute Acceleration with Admittance
    /**
     * admitance control: M * xdd + D * xd + S * x = F_ext
     * x_dd = M^{-1} * (D * x_d + weight * T * F_ext)
     * **/
    Vector6d arm_desired_accelaration_cartesian =
            mass_matrix.inverse() * ( -damping_matrix * x_dot + admittance_weight * (get_ee_rotation_matrix(joint_position, joint_velocity) * external_wrench));

    ROS_INFO_STREAM_NAMED(
            "Admittance Controller Output Acc",
            "Admittance Controller Output Acc: " << std::endl << std::endl << arm_desired_accelaration_cartesian << std::endl
    );

    // Integrate for Velocity Based Interface
    ros::Duration duration = loop_rate.expectedCycleTime();
    ROS_INFO_STREAM_ONCE(" Integrate Cycle Time: " << duration.toSec() * 1000 << " ms");
    x_dot += arm_desired_accelaration_cartesian * duration.toSec();

    if (!x_dot.allFinite()) x_dot.setZero();

    ROS_INFO_NAMED(
            "Desired Cartesian Velocity",
            "Desired Cartesian Velocity:  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            x_dot[0],
            x_dot[1],
            x_dot[2],
            x_dot[3],
...
//    }
    ...
    ROS_INFO_NAMED(
            ...
            "Desired Joints Velocity:     %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            q_dot[0],
            q_dot[1],
            q_dot[2],
            q_dot[3],
    ...
    ROS_INFO_NAMED(
            "Limited Desired Cartesian Velocity",
            "Limited Desired Cartesian Velocity:  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            x_dot_last_cycle[0],
            x_dot_last_cycle[1],
            x_dot_last_cycle[2],
            x_dot_last_cycle[3],
    ...
    ROS_INFO_NAMED(
            "Limited Desired Joints Velocity",
            "Limited Desired Joints Velocity:     %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
            q_dot[0],
            q_dot[1],
            q_dot[2],
            q_dot[3],
    ...
    if (complete_debug) {
        ROS_INFO_THROTTLE_NAMED(
                ...
                "Desired Cartesian Velocity",
                "Desired Cartesian Velocity:  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f",
                x_dot[0], x_dot[1], x_dot[2], x_dot[3], x_dot[4], x_dot[5]);
    }
    if (complete_debug) {
        ROS_INFO_THROTTLE_NAMED(
                2,
...
Vector6d admittance_control::limit_joint_dynamics(Vector6d jnt_velocity) {
    double duration = loop_rate.expectedCycleTime().toSec();
    // Limit Joint Velocity
    for (int i = 0; i < jnt_velocity.size(); i++) {
        if (fabs(jnt_velocity[i]) > max_vel[i]) {
            // ---- DEBUG ---- //
            if (complete_debug)
                ROS_INFO_NAMED(
                        "Reached Maximum Velocity",
                        "Reached Maximum Velocity on Joint %d   ->   Velocity: %.3f   Limited at: %.3f",
                        i, jnt_velocity[i], sign(jnt_velocity[i]) * max_vel[i]
                );
            jnt_velocity[i] = sign(jnt_velocity[i]) * max_vel[i];
        }
    }
    // Limit Joint Acceleration
    for (int i = 0; i < jnt_velocity.size(); i++) {
        if (fabs(jnt_velocity[i] - q_dot_last_cycle[i]) > max_acc[i] * duration) {
            // ---- DEBUG ---- //
            if (complete_debug)
                ROS_INFO_NAMED(
                        "Reached Maximum Acceleration",
                        "Reached Maximum Acceleration on Joint %d   ->   Acceleration: %.3f   Limited at: %.3f",
                        i, (jnt_velocity[i] - q_dot_last_cycle[i]) / duration, q_dot_last_cycle[i] + sign(jnt_velocity[i] - q_dot_last_cycle[i]) * max_acc[i]
                );
            jnt_velocity[i] = q_dot_last_cycle[i] + sign(jnt_velocity[i] - q_dot_last_cycle[i]) * max_acc[i] * duration;
        }
    }
    q_dot_last_cycle = jnt_velocity;
    return jnt_velocity;
}
...
void admittance_control::trajectory_execution(ur_admittance_controller::joint_trajectory desired_trajectory) {
    ...
    if (!force_extra_data_keypoint.empty()) force_keypoint_available = true;
    ...
    for (unsigned i = 0; i < trajectory.size() - 1; i++) {
        ...
        if (complete_debug) {
            ROS_INFO_NAMED(
                    "Trajectory Execution Joint Names",
                    "Trajectory Execution Joint Names: %s %s %s %s %s %s",
                    trajectory[i].name[0].c_str(),
                    trajectory[i].name[1].c_str(),
                    trajectory[i].name[2].c_str(),
                    trajectory[i].name[3].c_str(),
                    trajectory[i].name[4].c_str(),
                    trajectory[i].name[5].c_str()
            );
            ROS_INFO_NAMED(
                    "Trajectory Execution Velocity",
                    "Trajectory Execution Velocity: %.5f %.5f %.5f %.5f %.5f %.5f",
                    trajectory[i].velocity[0],
                    trajectory[i].velocity[1],
                    trajectory[i].velocity[2],
                    trajectory[i].velocity[3],
                    trajectory[i].velocity[4],
                    trajectory[i].velocity[5]
            );
        }
        // Sleep
    ...
    ROS_INFO("Restart Admittance Control\n");
}

void admittance_control::stop_robot() {
    Vector6d stop;
    stop.setZero();
...
void admittance_control::apply_force(double force_value) {
    std::cout << "\n";
    ROS_WARN("Force Application Requested: %.6f N", force_value);
...
sensor_msgs::JointState admittance_control::add_stop_point(std::vector<sensor_msgs::JointState> *trajectory) {
    ...
    stop_point.header.stamp = trajectory_temp[trajectory_temp.size() - 1].header.stamp +
                              (trajectory_temp[1].header.stamp - trajectory_temp[0].header.stamp);
...
std::vector<sensor_msgs::JointState> admittance_control::trajectory_scaling(ur_admittance_controller::joint_trajectory trajectory) {
    std::vector<sensor_msgs::JointState> input_trajectory = trajectory.trajectory, scaled_trajectory;
    std::vector<ur_admittance_controller::parameter_msg> extra_data = trajectory.extra_data;

    // ---- DEBUG ---- //
    if (simple_debug) {
        trajectory_debug_csv(input_trajectory, "input_trajectory");
    }
    ...
    for (unsigned i = 0; i < input_trajectory.size() - 1; i++) {
        input_positions.emplace_back(input_trajectory[i].position.data());
    }
    ...
    std::vector<tk::spline> q_spline6d = spline_interpolation(input_positions, trajectory_time, GET_VARIABLE_NAME(input_positions), complete_debug);
...
void admittance_control::check_requested_scaling(ur_admittance_controller::joint_trajectory *trajectory) {
    ...
    else if (percentage_vel != 100 && percentage_vel != 0) {
        ROS_INFO_STREAM("Velocity-Scaling Requested: " << percentage_vel << "%");
    }

}

double admittance_control::compute_scaled_velocity(ur_admittance_controller::joint_trajectory trajectory, double s_dot_rec) {
    ...
    // ---- DEBUG ---- //
    if (complete_debug) {
        ROS_INFO("s_dot_des = s_dot_rec * gain  ->  %.6f = %.6f * %.6f", s_dot_des, s_dot_rec, gain);
    }
...
std::vector<double> admittance_control::compute_s_des(double s_dot_des, double trajectory_time, double sampling_time, std::vector<ur_admittance_controller::parameter_msg> extra_data) {
    ...
    for (unsigned int i = 0; i < extra_data.size(); i++) {
        ...
        if (extra_data[i].parameter_name != "") {
            ...
            if (extra_data[i].parameter_name == "Accelerate") {
                velocity_extra_data_keypoint.push_back(new_extra_data_keypoint(2.0, s_value));
            } else if (extra_data[i].parameter_name == "Decelerate") {
                velocity_extra_data_keypoint.push_back(new_extra_data_keypoint(0.50, s_value));
            } else if (extra_data[i].parameter_name == "Force") {
                force_extra_data_keypoint.push_back(new_extra_data_keypoint(extra_data[i].parameter_value, s_value));
            }
        }
    }
    ...
    for (unsigned int i = 0; i < velocity_extra_data_keypoint.size(); i++) {
        ...
        // ---- DEBUG ---- //
        if (complete_debug) {
            ROS_INFO_STREAM("Scaling Velocity: " << s_dot_des << " -> From " << std::floor(
                    100 * velocity_extra_data_keypoint[i].time_keypoint / trajectory_time) << "% of trajectory");
        }
...
std::vector<Vector6d> admittance_control::compute_desired_positions(std::vector<double> s_des, std::vector<tk::spline> q_spline6d) {
    ...
    for (double s_de : s_des) {
        q_des.push_back(get_spline_value(q_spline6d, s_de));
    }
...
std::vector<Vector6d> admittance_control::compute_desired_velocities(std::vector<Vector6d> q_des, double sampling_time) {
    ...
    for (unsigned int i = 0; i < q_des.size() - 1; i++) {
        ...
        q_dot_des.emplace_back(q_dot_des_array);
...
std::vector<sensor_msgs::JointState> admittance_control::create_scaled_trajectory(std::vector<sensor_msgs::JointState> input_trajectory,
                                             std::vector<Vector6d> q_des, std::vector<Vector6d> q_dot_des,
...
void admittance_control::send_position_to_robot(Vector6d position) {
    ...
    if (switch_controller_client.call(switch_controller_srv)) {
        ROS_WARN(
                "Controller Changed from \"%s\" to \"%s\"",
                switch_controller_srv.request.stop_controllers[0].c_str(),
                switch_controller_srv.request.start_controllers[0].c_str()
        );
    ...
    trajectory_temp.joint_names = {"shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint",
                                   "wrist_2_joint", "wrist_3_joint"};
    trajectory_temp.points.resize(1);
    trajectory_temp.points[0].positions = {position[0], position[1], position[2], position[3], position[4],
                                           position[5]};
    ...
    ROS_INFO("GoTo Initial Position in %.6fs: %.6f %.6f %.6f %.6f %.6f %.6f", execution_time, position[0], position[1],
...
void admittance_control::wait_for_position_reached(Vector6d desired_position, double maximum_time) {
    ...
    while (ros::ok() && (Eigen::abs(desired_position.array() - current_position.array()) > 0.0001).all() &&
           (ros::Time::now() - start_time).toSec() < maximum_time) {
        ros::spinOnce();
...
void admittance_control::wait_for_callbacks_initialization() const {
    ros::spinOnce();
...
void admittance_control::ur_zero_ft_sensor() {
//    while (use_ur_real_robot && !ur_zero_ft_sensor_client.call(ur_zero_ft_sensor_srv)) {
//        ROS_WARN_THROTTLE(2, "Wait for Service: \"/ur_hardware_interface/zero_ftsensor\"");
//    }
//    if (ur_zero_ft_sensor_client.call(ur_zero_ft_sensor_srv)) ROS_INFO("Succesful Request \"zero_ftsensor\"\n");
}

void admittance_control::ur_send_script_command(std::string command) {
    // e-Series only
    std_msgs::String script_command;
    script_command.data = std::move(command);
    ur_script_command_publisher.publish(script_command);
}

void admittance_control::start_freedrive_mode() {
    ROS_INFO("FreeDrive Mode Activated");

    // Launch Freedrive Mode Script
    ur_send_script_command("def prog(): \
    freedrive_mode() \
    sleep(3600) \
    end");

    // Call zero_ftsensor service
    // e-Series only function
    ur_zero_ft_sensor();
...
void admittance_control::stop_freedrive_mode() {

    // Turn OFF FreedriveMode
    ROS_INFO("FreeDrive Mode Dectivated\n");
...
void admittance_control::ur_restart_urcap() {

    ros::Duration(2).sleep();

    // Restart URCap
    if (ur_resend_robot_program.call(ur_resend_robot_program_srv)) {}
    else ROS_ERROR("Failed to Call Service: \"/ur_hardware_interface/resend_robot_program\"");

    ros::Duration(2).sleep();

    if (ur_resend_robot_program.call(ur_resend_robot_program_srv)) ROS_INFO("Restarting URCAP Succesfull");
    else ROS_ERROR("Failed to Call Service: \"/ur_hardware_interface/resend_robot_program\"");

    // Play URCap
    // if (ur_play_urcap.call(ur_play_urcap_srv)) {ROS_INFO("Play URCAP Succesfull");}
    // else {ROS_ERROR("Failed to Call Service: \"/ur_hardware_interface/dashboard/play\"");}
}

/**************************************** UTILS FUNCTIONS ****************************************/

void admittance_control::publish_cartesian_position() {
    Eigen::Matrix4d ee_position = compute_fk();
...
Vector6d admittance_control::low_pass_filter(const Vector6d& input_vec) {
    // Adding new element to filter vector
    filter_elements.push_back(input_vec);
    // Keep only the last 100 values of the vector
    while (ros::ok() && filter_elements.size() > 100) {
        filter_elements.erase(filter_elements.begin());
    }
    // Median Filter (media = sum / N_elements)
    Vector6d sum, median;
    sum.setZero();
    median.setZero();
    for (const auto & filter_element : filter_elements) {
        sum += filter_element;
    }
...
void admittance_control::spinner() {
    ros::spinOnce();
    /** built-in FreeDrive Mode **/
    if (freedrive_mode_request) {
        ros::spinOnce();
